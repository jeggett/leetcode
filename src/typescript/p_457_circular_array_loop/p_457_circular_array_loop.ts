function getNextIndex(currentIndex: number, increment: number, arraySize: number): number {
    const newIndex = (currentIndex + increment) % arraySize;
    return newIndex < 0 ? arraySize + newIndex : newIndex;
}
function isNotCycle(nums: number[], currentPointer: number, initialDirection: number) {
    const directionsMismatch = initialDirection * nums[currentPointer] < 0;

    return (
        getNextIndex(currentPointer, nums[currentPointer], nums.length) === currentPointer ||
        directionsMismatch
    );
}
