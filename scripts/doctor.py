...
NODE_DEPENDENCIES = ("@biomejs/biome", "typescript", "vitest", "husky")
...
def husky_launcher_check(root: Path) -> Check:
    ...
    return Check("Husky launcher", True, "is readable and dispatches the tracked hook")
...
def husky_checks(root: Path, available: set[str], run: CommandRunner) -> list[Check]:
    ...
    if "git" not in available:
        ...
        hooks_path = _run(run, ("git", "config", "--get", "core.hooksPath"), root)
